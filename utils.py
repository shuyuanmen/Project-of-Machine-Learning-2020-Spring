...
# def get_LSTM_input(fname,down_fname,test_mode=False):
#
#     vec_up = read_excel(fname)
...
def get_LSTM_input(fname,test_mode=False):
    ...
    scalers = {}
    scalers['speed'] = vec_up['scaler_speed']
    scalers['flow'] = vec_up['scaler_flow']

    for i in range(len(speed_up)):
        if i+time_steps <= len(speed_up):
            feat_vec = features[i:i+time_steps].tolist()
            x.append(feat_vec)
            y_speed.append(speed_up[i+time_steps-1])
            y_flow.append(flow_up[i+time_steps-1])
            if test_mode:
                time_series_input[date[i+time_steps-1]] = feat_vec
